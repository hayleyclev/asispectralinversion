...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun  6 22:12:49 2026
...
def load_lookup_tables_directory(folder, maglat, plot=True):
    ...
    va = load_lookup_tables(fnamereda, fnamegreena, fnamebluea, fnamepeda, fnamehalla, maglat, plot=True)
...
def calculate_E0_Q_v2(redbright, greenbright, bluebright, inlookup_table, minE0=150, generous=False):
    ...
    qvec, maxqvec, minqvec = q_interp(lookup_table['bluemat'], lookup_table['Qvec'], lookup_table['E0vec'], bluevec, minE0ind=minE0ind, maxbluebright='auto', interp='linear', plot=True)
...
def q_interp(bright428 ,Qvec, E0vec, bluevec, minE0ind=0, maxbluebright='auto', interp='linear', plot=True):   
    ...
    if maxbluebright == 'auto':
        ...
        _, testmaxqvec, _ =  q_interp(bright428, Qvec, E0vec, testbluevec, minE0ind=minE0ind, maxbluebright=np.inf, interp=interp, plot=True)
