...
def process_grouped_files(date, starttime, endtime, maglatsite, folder, base_outdir, lambdas, next_group_idx):
    """
    Purpose:
    ...
    # Check the maximum number of groups across all wavelengths
    max_groups = max(len(files) for files in grouped_files.values())
    
    print(f"Processing groups from {next_group_idx + 1} to {max_groups}")
    
    # Iterate over each group index
    for idx in range(next_group_idx, max_groups):
        foi_files = {}
        ...
        for lam in lambdas:
            if idx < len(grouped_files[lam]):
                ...
                foi_files[lam] = None  # handle cases where there may be fewer files for some wavelengths
        ...
        if all(foi is not None for foi in foi_files.values()):
            ...
            print(f"Skipping group {idx + 1} due to missing files.")
            continue
