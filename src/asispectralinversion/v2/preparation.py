...
def prepare_data(date, maglatsite, folder, foi_0428, foi_0558, foi_0630, group_outdir, group_number):
    ...
    # create artifact removal here for red image, new variable will be redimcoadd_interp
    redimcoadd_interp = remove_artifacts(redimcoadd)
    plt.imshow(redimcoadd_interp)
    plt.title('Red Imagery (No Artifacts)')
    plt.xlabel('E-W')
    plt.ylabel('N-S')
    red_interp_fn = 'red_imagery_interp.png'
    red_interp_out = os.path.join(group_outdir, red_interp_fn)
    plt.savefig(red_interp_out)
    plt.close()
    # call from artifact_removing.py
    # show plot of red image with interpolation over the selected area
    ...
    plt.close()
     ...
     # create artifact removal here for green image, new variable will be greenimcoadd_interp
    greenimcoadd_interp = remove_artifacts(greenimcoadd)
    plt.imshow(greenimcoadd_interp)
    plt.title('Green Imagery (No Artifacts)')
    plt.xlabel('E-W')
    plt.ylabel('N-S')
    green_interp_fn = 'green_imagery_interp.png'
    green_interp_out = os.path.join(group_outdir, green_interp_fn)
    plt.savefig(green_interp_out)
    plt.close()
     # call from artifact_removing.py
     # show plot of green image with interpolation over the selected area   
    ...
    # create artifact removal here for blue image, new variable will be blueimcoadd_interp
    blueimcoadd_interp = remove_artifacts(blueimcoadd)
    plt.imshow(blueimcoadd_interp)
    plt.title('Blue Imagery (No Artifacts)')
    plt.xlabel('E-W')
    plt.ylabel('N-S')
    blue_interp_fn = 'blue_imagery_interp.png'
    blue_interp_out = os.path.join(group_outdir, blue_interp_fn)
    plt.savefig(blue_interp_out)
    plt.close()
    # call from artifact_removing.py
    # show plot of blue image with interpolation over the selected area
    ...
    # replace add <color>imcoadd with <color>imcoadd_interp
    blueimdenoisewavelet, blueimreg, lon110, lat110, maglon, maglat, bluebgbright, bluesig = wavelet_denoise_resample(blueimcoadd_interp, 
                                                                                                                      dtdate, 
                                                                                                                      skymapblue[1], 
                                                                                                                      skymapblue[0], 
                                                                                                                      interplonvec, 
    ...
    # replace add <color>imcoadd with <color>imcoadd_interp
    blueimdenoisegauss, _, _, _, _, _, _, _ = gaussian_denoise_resample(blueimcoadd_interp, 
                                                                        dtdate, 
                                                                        skymapblue[1], 
                                                                        skymapblue[0], 
                                                                        interplonvec, 
    ...
    # replace add <color>imcoadd with <color>imcoadd_interp
    redimdenoise, redimreg, _, _, _, _, redbgbright, redsig = wavelet_denoise_resample(redimcoadd_interp, 
                                                                                       dtdate, 
                                                                                       skymapred[1], 
                                                                                       skymapred[0], 
                                                                                       interplonvec, 
    ...
    # replace add <color>imcoadd with <color>imcoadd_interp
    greenimdenoise, greenimreg, _, _, _, _, greenbgbright, greensig = wavelet_denoise_resample(greenimcoadd_interp, 
                                                                                               dtdate, 
                                                                                               skymapgreen[1], 
                                                                                               skymapgreen[0], 
                                                                                               interplonvec, 
