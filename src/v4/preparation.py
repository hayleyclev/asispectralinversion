...
from inversion import calculate_E0_Q_configurable
...
def prepare_data(date, maglatsite, folder, foi_0428, foi_0558, foi_0630, group_outdir, group_number, inversion_config=None, skymap_path=None):
    """
    Purpose:
    ...
    v = load_lookup_tables_directory(folder, maglat=maglatsite, use_maglat_conductance_correction=inversion_config.get("use_maglat_conductance_correction", True),
                                     load_edens=inversion_config.get("load_edens", True), use_airglow=True, plot=inversion_config.get("plot", False))
    ...
    if skymap_path is None:
        raise RuntimeError("prepare_data requires skymap_path.")

    if not os.path.exists(skymap_path):
        raise RuntimeError(f"Could not find skymap.mat:\n{skymap_path}")

    vskymap = copy_h5(h5py.File(skymap_path, "r")["magnetic_footpointing"])
    ...
    greenimcoadd_original = (greenims.get('original_frame_1', greenims['frame_1']) + 
                             greenims.get('original_frame_2', greenims['frame_2']) + 
                             greenims.get('original_frame_3', greenims['frame_3'])) / 3

    blueimcoadd_original = (blueims.get('original_frame_1', blueims['frame_1']) + 
                            blueims.get('original_frame_2', blueims['frame_2']) + 
                            blueims.get('original_frame_3', blueims['frame_3'])) / 3

    redimcoadd_original = (redims.get('original_frame_1', redims['frame_1']) + 
                           redims.get('original_frame_2', redims['frame_2']) + 
                           redims.get('original_frame_3', redims['frame_3'])) / 3
    ...
    blueimdenoisewavelet, blueimreg, lon110, lat110, maglon, maglat, bluebgbright, bluesig = wavelet_denoise_resample(blueimcoadd_interp, dtdate, skymapblue[1], 
                                                                                                                      skymapblue[0], interplonvec, interplatvec, 
                                                                                                                      110, nshifts=n_shifts, background_method=background_method, 
                                                                                                                      plot=False)

    blueimdenoisegauss, _, _, _, _, _, _, _ = gaussian_denoise_resample(blueimcoadd_interp, dtdate, skymapblue[1], skymapblue[0], interplonvec, interplatvec,
                                                                        110, blur_deg_EW, NS_deg=blur_deg_NS, plot=False)
    ...
    redimdenoise, redimreg, _, _, _, _, redbgbright, redsig = wavelet_denoise_resample(redimcoadd_interp, dtdate, skymapred[1], skymapred[0], interplonvec, 
                                                                                       interplatvec, 110, nshifts=n_shifts, background_method=background_method, 
                                                                                       plot=False)

    greenimdenoise, greenimreg, _, _, _, _, greenbgbright, greensig = wavelet_denoise_resample(greenimcoadd_interp, dtdate, skymapgreen[1], skymapgreen[0], 
                                                                                               interplonvec, interplatvec, 110, nshifts=n_shifts, 
                                                                                               background_method=background_method, plot=False)
    ...
    colormat = np.asarray([nred * (redframe - redmin),
                           ngreen * (greenframe - greenmin), 
                           nblue * (blueframe - bluemin)]).astype(float)
    ...
    qout, e0out, minq, maxq, mine0, maxe0 = calculate_E0_Q_configurable(redraydec, greenraydec, blueraydec, v, config=inversion_config)

    print("Calculating conductivities given Q and E0...")

    SigP, SigH = calculate_Sig(qout, e0out, v, generous=inversion_config.get("generous", False))
